...
public class TetroITests {

	@Test
	public void testI() {
		Tetromino t = new TetroI('N');
		...
		/*First Rotation*/
/*		assertEquals(new Coords(a0.X() + 2, a0.Y() - 1), a1);
		assertEquals(new Coords(b0.X() + 1, b0.Y() - 0), b1);
		assertEquals(new Coords(c0.X() + 0, c0.Y() + 1), c1);
		assertEquals(new Coords(d0.X() - 1, d0.Y() + 2), d1);
		
		a0 = new Coords(a1); b0 = new Coords(b1); c0 = new Coords(c1); d0 = new Coords(d1);
		g.getCurr().rotate(g.getPlaced());
		/*Second Rotation*/
/*		assertEquals(new Coords(a0.X() + 1, a0.Y() + 2), a1);
		assertEquals(new Coords(b0.X() + 0, b0.Y() + 1), b1);
		assertEquals(new Coords(c0.X() - 1, c0.Y() + 0), c1);
		assertEquals(new Coords(d0.X() - 2, d0.Y() - 1), d1);
